...
- ✅ 500 esporádico em `POST /api/v1/conversations` mitigado: quando `/contacts/{id}/conversations` não retorna uma conversa existente, a bridge procura também por `/conversations?status=all&source_id=...`; se a criação autenticada falhar, tenta criar pelo endpoint público do inbox API usando o `source_id` já existente, relê por `source_id`/contato, reabre se necessário e continua postando a mensagem quando consegue recuperar.
...
- **Canal da Wuzapi podia responder `no session` no envio** — quando o retry de texto ainda volta com `no session`, a bridge tenta reconectar a sessão do token em `/session/connect` e reenvia uma vez. Se a Wuzapi pedir novo QR ou a sessão estiver inválida, o debug passa a registrar a tentativa de reconnect para facilitar a ação manual.
